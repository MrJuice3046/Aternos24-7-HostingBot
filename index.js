...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sidebar { width: 260px; background: var(--card); padding: 2rem 1.5rem; display: flex; flex-direction: column; border-right: 1px solid var(--border); position: fixed; height: 100vh; z-index: 10; }
...
app.post('/restart', (req, res) => { res.json({success:true}); setTimeout(() => process.exit(0), 500); });
app.post('/disconnect', (req, res) => { if (bot) bot.end(); res.json({success:true}); });
app.post('/reconnect', (req, res) => { if (bot) bot.end(); intentionalDisconnect = false; createBot(); res.json({success:true}); });

// ---- Server ----
const server = app.listen(PORT, '0.0.0.0', () => console.log(`[Server] Dashboard on port ${server.address().port}`));
server.on('error', (err) => { if (err.code === 'EADDRINUSE') server.listen(PORT + 1, '0.0.0.0'); else console.error(err); });
...
// ---- Memory ----
setInterval(() => { console.log(`[Memory] ${(process.memoryUsage().heapUsed/1024/1024).toFixed(1)} MB`); }, 5 * 60 * 1000);

// ============================================================
// BOT LOGIC – TRULY IMMORTAL
// ============================================================
...
function getReconnectDelay() {
  if (botState.wasThrottled) { botState.wasThrottled = false; return 60000 + Math.floor(Math.random() * 60000); }
  const timeSinceDeath = Date.now() - lastDeathTime;
  const isRecentDeath = (lastDeathTime > 0 && timeSinceDeath < 120000);
  // 1 minute after death, otherwise 30 seconds
  const base = config.utils['auto-reconnect-delay'] || (isRecentDeath ? 60000 : 30000);
...
function setupKeepAliveHandler() {
  ...
  bot.on('keepalive', (payload) => { if (bot && bot._client) bot._client.write('keepalive', { id: payload }); });
}
...
function createBot() {
  ...
  try {
    ...
    // Connection timeout – if no spawn within 2 minutes, kill and retry
    connectionTimeoutId = setTimeout(() => {
      if (!botState.connected) {
        console.log('[Bot] Connection timeout (2 min) – will retry');
        try { bot.removeAllListeners(); bot.end(); } catch (e) {}
        bot = null;
        scheduleReconnect();
      }
    ...
    bot.once('spawn', () => {
      ...
      console.log(`[Bot] Successfully spawned (${bot.version})`);
    ...
    // ✅ EVERY ERROR FORCES A CLEAN DISCONNECT
    bot.on('error', (err) => {
      console.log('[Bot] Error:', err.message);
      // Timeout or any fatal error → force reconnect
      if (err.message.includes('timed out') || err.message.includes('keepalive') || err.message.includes('ECONNRESET') || err.message.includes('EPIPE')) {
        console.log('[Bot] Fatal error, forcing disconnect');
        try { bot.end(); } catch (e) {}
      }
    });
...
// ============================================================
// WATCHDOG – ensures the bot never stays dead
// ============================================================
setInterval(() => {
...
function startLookAround(bot) {
  addInterval(() => { try { bot.look(Math.random()*Math.PI*2 - Math.PI, Math.random()*Math.PI/2 - Math.PI/4, false); } catch(e) {} }, config.movement['look-around'].interval);
}
...
console.log('  Minecraft AFK Bot – Truly Immortal');
console.log('═'.repeat(45));
createBot();
