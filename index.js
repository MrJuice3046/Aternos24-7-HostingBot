...
<head>
  ...
  <style>
    :root { --bg: #0f1117; --card: #1a1d27; --accent: #6366f1; --text: #e2e8f0; --text-dim: #94a3b8; --border: #2d3143; --green: #22c55e; --red: #ef4444; }
...
// ============================================================
// BOT LOGIC – INSTANT CREATIVE MODE (NO DEATH)
// ============================================================
...
function pingServer() {
  return new Promise((resolve, reject) => {
    mc.ping({ host: config.server.ip, port: config.server.port, version: config.server.version || false }, (err, res) => err ? reject(err) : resolve(res));
  });
}
...
function createBot() {
  ...
  try {
    ...
    bot.once('spawn', () => {
      ...
      // ✅ INSTANT CREATIVE MODE (no fall damage, no death)
      if (bot.creative && bot.creative.setCreative) {
        bot.creative.setCreative(true);
        console.log('[Bot] Creative mode enabled (via API)');
      }
...
function initializeModules(bot, mcData, defaultMove) {
  ...
  if (isModuleEnabled('utils.chat-messages.enabled')) {
    ...
    if (config.utils['chat-messages'].repeat) {
      let i = 0;
      addInterval(() => {
        if (bot && botState.connected) { bot.chat(msgs[i]); i = (i + 1) % msgs.length; }
...
process.on('unhandledRejection', (reason) => { console.log('[FATAL] Rejection:', reason); });
...
console.log('  Minecraft AFK Bot – Instant Creative Mode');
console.log('═'.repeat(45));
